#!/usr/bin/env bats
#
# Tests for list_dependencies_optional script.
...
@test "list_dependencies_optional: shows usage with no arguments" {
  run ./list_dependencies_optional
  [ "$status" -eq 1 ]
  [[ "$output" == *"Usage:"* ]]
}

@test "list_dependencies_optional: filters out required deps" {
...
JSON
  cd "$TEST_DIR"
  run "$BATS_TEST_DIRNAME/../list_dependencies_optional" extensions/Echo
  [ "$status" -eq 0 ]
  expected=$(printf "B\nC")
  [ "$output" = "$expected" ]
}

@test "list_dependencies_optional: all deps optional when no requires" {
  cat > "$TEST_DIR/src/config/zuul/dependencies.yaml" <<'YAML'
Echo:
  - A
  - B
...
@test "list_dependencies_optional: no optional deps when all are required" {
  cat > "$TEST_DIR/src/config/zuul/dependencies.yaml" <<'YAML'
Echo:
  - A
YAML
