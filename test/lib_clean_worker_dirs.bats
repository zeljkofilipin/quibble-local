#!/usr/bin/env bats
#
# Tests for lib/clean_worker_dirs.
...
@test "clean_worker_dirs: no error when no worker dirs exist" {
  run bash -c '
    cd "'"$test_dir"'"
    . "'"$project_root"'/lib/clean_worker_dirs"
  '
