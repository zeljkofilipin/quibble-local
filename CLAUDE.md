...
- **Slow integration test:** `./test_integration_slow` — runs tests that are too slow for the fast suite (network-heavy setup, full test suites, exhaustive algorithms, gated repository tests). Slow but deterministic — failures indicate real regressions in this repo. Kept separate from `./test_integration` so that suite stays fast.
...
- Integration test entries are split across three suites by observed runtime and reliability: `test_integration` (fast — ~10 minutes total, each entry ≤1 minute, deterministic), `test_integration_slow` (slow — tests too slow for the fast suite, deterministic), and `test_integration_flaky` (flaky — intermittent failures driven by upstream issues, regardless of speed). Categorize by **observed instability**, not naming: many `*_gated` scripts are reliable and live in `test_integration_slow`. Annotate each entry's approximate runtime in its inline comment as `# Nm: ...` or `# <1m: ...`.
