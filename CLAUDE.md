...
- **Unit test:** `bats test/` — runs Bats unit tests (fast, no Docker needed)

There is no build step. `./integration_test` is an integration test that exercises every script. Unit tests use [Bats](https://github.com/bats-core/bats-core) (Bash Automated Testing System). All scripts should have Bats tests where possible. Bats tests go in the `test/` directory.
...
- When adding or updating a script, ensure `README.md` and `integration_test` are kept in sync, and run `./lint`. `lint` and `help` auto-discover scripts (by checking the first line for a bash shebang or shellcheck directive), so they do not need manual updates when adding new scripts.
