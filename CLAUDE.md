...
- Long-running scripts (`dependencies_minimal`, `dependencies_minimal_bottom_up`, `dependencies_minimal_gated`, `dependencies_minimal_thorough`, `run_selenium_tests_all_gated`, `run_selenium_tests_required_gated`, `test_integration`, `test_integration_slow`) source `lib/inhibit_sleep` to prevent the machine from suspending
