...
- **Gated integration test:** `./test_integration_gated` — runs upstream-fragile scripts (e.g. `run_selenium_tests_gated`) that depend on the full set of gated Wikimedia repositories being healthy upstream. Kept separate from `./test_integration` so that suite stays fast and reliable.
...
- Long-running scripts (`dependencies_minimal`, `dependencies_minimal_bottom_up`, `dependencies_minimal_gated`, `dependencies_minimal_thorough`, `run_selenium_tests_all_gated`, `run_selenium_tests_required_gated`, `test_integration`, `test_integration_slow`, `test_integration_gated`) source `lib/inhibit_sleep` to prevent the machine from suspending
...
- Long-running scripts that can be scoped to a single component should be tested with one component (e.g. `./run_selenium_tests_all_gated extensions/Echo`). Scripts that cannot be scoped and are already tested implicitly by another integration test entry do not need their own entry (e.g. `install_all_gated` is tested by `run_selenium_tests_gated`, which lives in `test_integration_gated` rather than `test_integration` because it depends on the full set of gated repos being healthy upstream).
