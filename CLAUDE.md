...
- All bash scripts start with `set -euo pipefail` and source `lib` (which enables `set -x` with a custom `PS4`). Exception: utility scripts (`help`, `lint`, `test`, `deep_test`, `dependencies`, `gated`, `selenium_tests_exist`, `run_all`) skip `lib` because trace output would bury their actual output in noise.
...
- Internal scripts (sourced helpers like `lib`, `inhibit_sleep`) must be documented in a separate section in `README.md` to make it clear they are not intended to be run directly
