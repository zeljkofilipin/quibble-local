...
- Long-running scripts (`dependencies_minimal`, `run_selenium_tests_all`, `run_selenium_tests_required`, `test_integration`) source `lib/inhibit_sleep` to prevent the machine from suspending
...
- `src_worker_*/` — isolated src directories for parallel workers (created by `PARALLEL=N ./dependencies_minimal`)
