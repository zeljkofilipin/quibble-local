...
- **Integration test:** `./test_integration` — runs all scripts end-to-end (slow; requires Docker, clones repos from Gerrit)
- **Gated integration test:** `./test_integration_gated` — runs long-running and upstream-fragile scripts (e.g. `run_selenium_tests_gated`) that depend on the full set of gated Wikimedia repositories being healthy upstream. Kept separate from `./test_integration` so that suite stays fast and reliable.
...
- Long-running scripts (`dependencies_minimal`, `dependencies_minimal_bottom_up`, `dependencies_minimal_gated`, `dependencies_minimal_thorough`, `run_selenium_tests_all_gated`, `run_selenium_tests_required_gated`, `test_integration`, `test_integration_gated`) source `lib/inhibit_sleep` to prevent the machine from suspending
